...
public interface WorkspaceRepository extends JpaRepository<WorkspaceDto, Long> {
	...
	@Query(value = "SELECT * FROM workspace INNER JOIN user_has_workspace ON  workspace.id=user_has_workspace.workspace_id  AND id=?1  AND delete_status=0 ", nativeQuery = true)
